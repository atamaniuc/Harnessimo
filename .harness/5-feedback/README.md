...
**1. The rules.** Unit tests over `src/`, each rule with at least one test proving it fires
on bad input. The rules are pure functions, so this level needs no fixture tree and takes
under a second.

**2. The wiring.** `test/cli.test.ts` builds real repositories in a temporary directory
...
**3. The whole thing on itself.** `node src/cli.ts check` runs every gate this
...
`node src/cli.ts check --reverify` re-runs the verification of every item claiming to
