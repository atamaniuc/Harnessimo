...
**1. The rules.** 73 tests over `src/`, each rule with at least one test proving it fires
...
**3. The whole thing on itself.** `node bin/harness.mjs check` runs every gate this
repository defines against this repository. `harness cold-start` then does it again in a
fresh clone, with nothing carried over.

## Re-checking claims

`node bin/harness.mjs check --reverify` re-runs the verification of every item claiming to
