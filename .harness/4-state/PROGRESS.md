...
1. **Spec 0003, doc contracts — all three phases shipped.** Every page now
   carries a contract in `docs/contracts.json` — the question it answers, its reader, and
   the `excludes` list that makes overlap a failing test rather than a matter of taste.
   Pages open with who they are for; the README routes by question instead of listing page
   names; `FAQ.md` and `TROUBLESHOOTING.md` exist, in both languages, built from failures
   this repository actually had. Phase 3 closed the last two gaps: `CONFIGURATION.md`,
   generated from the JSON schema in both languages, and `llms.txt` — every page as one
   file for the reader profile that is not a person. Both are generated and drift-checked;
   six schema keys had no description at all and now do.
2. **Spec 0002, docs consolidation — half shipped, one decision open.** `ADOPTING.md` is
