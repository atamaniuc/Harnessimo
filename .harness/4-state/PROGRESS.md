...
- Zero runtime dependencies. Written in TypeScript (strict); Node runs the sources
  ...
  check types.
- On npm as `harnessimo`, published by `.github/workflows/publish.yml` through **trusted
  publishing (OIDC)** with provenance. No stored npm token exists; renaming that workflow
  file breaks publishing until the publisher is updated, which is the trade.
- Documentation is single-sourced and bilingual: `README.md` / `README.ru.md` generate the
  site's home pages via `scripts/site-home.ts`, `npm run check` fails on drift, and
  `test/translations.test.ts` fails on a page that exists in one language only. Every page
  carries a contract in `docs/contracts.json`, which makes overlap a failing test.
- The rules are pure functions in `src/`; `src/cli.ts` and `src/resolver.ts` are the
  only code that touches the world.
- **Both source repositories depend on this package** and have deleted their copies.
