...
import { checkTarget, findMarkers, isSyntaxExample, verifyProofs } from "../src/proof.ts";
import { fakeResolver, reason } from "./helpers.ts";
...
test("a target that is only a path checks the file exists", () => {
  ...
  assert.match(reason(checkTarget("src/gone.ts", resolver)), /does not exist/);
});
...
test("a path:symbol target checks the file contains the symbol", () => {
  ...
  assert.match(reason(checkTarget("src/search.ts:missingSymbol", resolver)), /does not contain/);
});
...
test("a path#test-name target checks the named test still exists", () => {
  ...
  assert.match(reason(checkTarget("test/rls.spec.ts#a renamed test", resolver)), /does not contain/);
});
...
test("a command target resolves against the runner that declares it", () => {
  ...
  assert.match(reason(checkTarget("task deploy", resolver)), /no task command named "deploy"/);
});
...
test("a command kind the project does not declare says so instead of passing", () => {
  ...
  assert.match(reason(checkTarget("just build", resolver)), /declares no "just" command file/);
});

test("a migration target matches by prefix", () => {
  assert.equal(checkTarget("migration:20260821110000", resolver), null);
  assert.match(reason(checkTarget("migration:19990101", resolver)), /no migration starts with/);
});
