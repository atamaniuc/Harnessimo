...
export function lockedViolations(commits, { paths, agentTrailer, exempt = new Set() }) {
    ...
    for (const commit of commits) {
        if (exempt.has(commit.sha))
            continue;
        if (!commit.message.includes(agentTrailer))
            continue; // a human commit is allowed to change these
        const hits = commit.files.filter((file) => paths.some((locked) => file.startsWith(locked)));
        if (hits.length > 0)
            violations.push({ sha: commit.sha, hits });
    }
    return violations;
}
export function formatLockedViolations(violations) {
    return violations
        .map((v) => `locked surface modified by an agent commit: ${v.sha}\n` +
        v.hits.map((h) => `  ${h}`).join("\n") +
