...
export function debrisProblems(files, options = {}) {
    ...
    for (const file of files) {
        if (allow.some((pattern) => file.path.includes(pattern)))
            continue;
        file.text.split("\n").forEach((lineText, index) => {
            for (const marker of markers) {
                if (!lineText.includes(marker))
                    continue;
                if (isWordMarker(marker) && !isAnnotation(lineText, marker))
                    continue;
                problems.push({
                    file: file.path,
                    line: index + 1,
                    target: marker,
                    reason: `left behind by a session that called itself finished — a marker like this compiles, ` +
...
export function progressProblems({ changed, progressFile, codePrefixes = [], changedLines = null, threshold = 50, }) {
    ...
    if (changed.length === 0)
        return [];
    if (changed.includes(progressFile))
        return [];
    const touched = codePrefixes.length === 0
    ...
    return [
        {
            ...
            line: 1,
            target: "(not updated)",
            reason: `${changed.length} file(s) changed and this was not one of them — the next session reads ` +
    ...
    for (const file of files) {
        ...
        if (lines > limit) {
            problems.push({
                ...
                line: limit + 1,
                target: `${lines} lines`,
                reason: `the instruction file is ${lines - limit} line(s) over its ${limit}-line limit — a long ` +
...
//# sourceMappingURL=cleanexit.js.map