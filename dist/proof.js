 ...
 *
 */
export function checkTarget(target, resolver) {
 ...
 *
 */
export function isSyntaxExample(target) {
    return /\.\.\.|[[\]<>|]/.test(target);
}
 ...
 *
 */
export function verifyProofs(files, resolver, options = {}) {
    ...
    for (const file of files) {
        ...
        if (strict && mustCarryProof.includes(file.path) && markers.length === 0) {
            problems.push({
                ...
                line: 1,
                target: "(none)",
                reason: "carries no proof marker; a claim with no evidence at all is the defect this gate exists for",
            });
        }
