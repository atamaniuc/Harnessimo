...
import { loadConfig, enabledChecks, ConfigError, CONFIG_FILE } from "./config.js";
import { collectDocs, collectFiles, createResolver, readAll } from "./resolver.js";
import { findMarkers, verifyProofs } from "./proof.js";
import { checkHandoffRefs, checkTracks, trackLines } from "./tracks.js";
import { verifyTaskGates } from "./tasks.js";
import { liveTrackSpecDirs } from "./tracks.js";
import { HarnessError, activate, checkQueue, formatStatus, lastLines, verifyItem, } from "./queue.js";
import { formatLockedViolations, lockedViolations } from "./locked.js";
import { coldStartProblems } from "./coldstart.js";
import { debrisProblems, instructionProblems, progressProblems } from "./cleanexit.js";
import { detectConfig } from "./detect.js";
import { HOOKS_DIR, HOOK_PATH, hookScript, hookStatus } from "./hooks.js";
import { briefJson, briefText, handoffPaths, mergeSessionStartHook } from "./brief.js";
...
function requireSection(cfg, section, hint) {
    ...
    if (!value) {
        die(`harnessimo: this repository has no "${section}" section in harnessimo.config.json, so there is nothing to check\n` +
            `  fix:  ${hint}`);
    }
    return value;
}
    ...
    return (cmd) => {
        try {
            ...
            if (error.signal === "SIGTERM") {
                return {
                    ok: false,
                    output: `verification exceeded ${timeoutMinutes} minutes and was killed\n` +
...
function runTasks() {
    ...
    if (!tracks.gateTasks)
        return { problems: [], summary: "task gating is off (tracks.gateTasks=false)" };
    const docs = cfg.docs ??
        { roots: [tracks.specsDir], skip: [], maxDepth: 3, mustCarryProof: [], commands: {}, migrations: null };
    const resolver = createResolver(ROOT, docs);
    if (!resolver.fileExists(tracks.file))
        return { problems: [], summary: "no track index, nothing to gate" };
    ...
    return {
        problems,
        summary: `checked boxes in ${gated.length} live task list(s) name a passing check` +
...
function runQueueCheck({ reverify } = {}) {
    const cfg = config();
    if (!cfg.queue)
        return { problems: [], summary: "no queue configured" };
    const withQueue = cfg;
...
function runCleanExit(base, head) {
    ...
    if (!clean.scan.length)
        return { problems: [], summary: "no paths configured to scan" };
    let changed = [];
    try {
    ...
    try {
        for (const row of git("diff", "--numstat", `${base}..${head}`).split("\n").filter(Boolean)) {
            ...
            if (path)
                changedLines[path] = (Number(added) || 0) + (Number(removed) || 0);
        }
    }
    catch {
        /* leave it empty: the rule then falls back to firing on any code change */
    }
    problems.push(...progressProblems({
        changed,
...
function cmdCheck() {
    const cfg = config();
    if (!cfg.path) {
        die("harnessimo: no harnessimo.config.json in this repository\n" +
            "  fix:  run `harnessimo init` to scaffold the harness, or add the config by hand");
    }
    const enabled = enabledChecks(cfg);
    const results = [];
    if (enabled.proof)
        results.push(["proof markers", runProof({ strict: true })]);
    if (enabled.tracks)
        results.push(["work tracks", runTracks()]);
    if (enabled.tasks)
        results.push(["task gate", runTasks()]);
    if (enabled.queue)
        results.push(["queue", runQueueCheck({ reverify: flags.has("--reverify") })]);
    if (enabled.instructions)
        results.push(["instructions", runInstructions()]);
    let failed = 0;
...
function cmdDoctor() {
    const cfg = config();
    if (!cfg.path)
        die("harnessimo: no harnessimo.config.json here.\n  fix:  run `harnessimo init`");
    const enabled = enabledChecks(cfg);
...
function cmdQueue() {
    ...
    try {
        if (sub === "status")
            return ok(formatStatus(list));
        if (sub === "activate") {
            if (!id)
                die("harnessimo: `harnessimo queue activate <id>` needs an id");
        ...
        if (sub === "verify") {
            if (!id)
                die("harnessimo: `harnessimo queue verify <id>` needs an id");
        ...
        if (sub === "check") {
            ...
            if (result.problems.length > 0)
                die(formatProblems(result.problems));
...
function cmdLocked() {
    ...
    if (!locked.paths.length)
        return ok("harnessimo: no locked paths configured, nothing to protect");
    ...
    if (violations.length > 0)
        die(formatLockedViolations(violations));
...
function cmdColdStart() {
    ...
    if (!cold.commands.length)
        return ok("harnessimo: no cold-start commands configured");
    ...
    try {
        ...
        if (error?.status !== undefined)
            die(`cold start FAILED: a documented command exited ${error.status}`);
        throw caught;
    }
    finally {
