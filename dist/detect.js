...
export function detectConfig(repo) {
    ...
    if (mustCarryProof.length === 0) {
        missing.push(candidates.length === 0
    ...
    const lockedPaths = [];
    if (repo.isDir(".github/workflows"))
        lockedPaths.push(".github/workflows/");
    if (repo.exists(".harness/1-instructions/CONSTRAINTS.md"))
        lockedPaths.push(".harness/1-instructions/CONSTRAINTS.md");
    ...
    const commandsToRun = [setup, checkCommand].filter((c) => Boolean(c));
