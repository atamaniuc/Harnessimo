...
// project, not a gate.
import { checkTarget, isSyntaxExample } from "./proof.js";
...
export function checkTaskGate(text, filePath, resolver) {
    ...
    for (const block of taskBlocks(text)) {
        ...
        if (!CHECKED_TASK.test(firstLine))
            continue;
        ...
        if (markers.length === 0) {
            problems.push({
                ...
                line,
                target: "(none)",
                reason: "checked task names no executable check — add <!-- proof: ... --> naming the test, eval case, or query that proves it",
            });
            continue;
        }
