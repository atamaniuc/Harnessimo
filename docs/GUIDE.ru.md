...
Не включайте все одиннадцать сразу. Возьмите ту, что отвечает реальной боли, добейтесь зелёного,
...
| Ключ, цвет или импорт не на своём месте | `boundaries` | этот шаблон появился там, где вы его запретили |
| Число качества, которое тихо просело | `thresholds` | метрика ниже объявленного для неё пола |
...
## 4b. Открыть и закрыть лан { #lanes }

Половина про SDD. Лан — это нумерованный каталог, чья спека несёт критерии, умеющие
запускаться, а проверки — то, что отказывается считать его законченным, пока они не проходят.

```bash
harnessimo track new checkout-totals --title "Checkout totals"
```

Следующий свободный номер, каталог, `spec.md`, `tasks.md` и `handoff.md` из шаблонов и
строка в индексе треков — той самой, которую `harnessimo brief` читает в начале каждой
сессии, чтобы лан не мог существовать на диске и быть невидимым для следующей сессии.

```bash
harnessimo track close checkout-totals --outcome "Итоги округляются по валюте."
```

Итог уходит наверх лога, строка индекса исчезает, хендофф удаляется. Спека и задачи
остаются: их несёт git.

Команда пишет **только механическое**. Она не сочиняет вашу спеку, не судит её форму и не
имеет мнения о том, что должно быть в лане, — генератор, который пишет прозу, превращает
ваше соглашение в собственность инструмента. Правило: автоматизируй то, в чём человек
ошибается одинаково; оставь то, в чём он ошибается по-разному.
...
## 6b. Больше одного агента { #parallel }

Как только работа идёт несколькими агентами — субагенты в одной сессии, параллельные
worktree, headless-прогон рядом с человеком, — полоса объявляет, какие пути она держит,
`harnessimo tracks` падает, когда две живые полосы заявили одни и те же, а
`hooks install --agent` не даёт закончить ход при красном `check`.
Всё это — в [«Больше одного агента»](PARALLEL.ru.md).
...
`--reverify` перезапускает каждый пункт, который заявляет `passing`. Именно эта строка
превращает доказательства очереди в доказательства, а не в строку, которую кто-то напечатал.

Или скажите, чем является CI, и пусть уровень решит сам:

```yaml
- run: npx harnessimo check --autonomy unattended --range "${{ github.event.pull_request.base.sha }}..HEAD"
```

`unattended` означает, что не смотрел никто, — поэтому запускается всё: переперепроверка,
проверка закрытых поверхностей, чистый выход и холодный старт. Нижнюю границу для
повседневной работы объявите в `harnessimo.config.json` — `{ "autonomy": { "level": "reviewed" } }` —
и пусть CI её поднимает. Опустить её нельзя.
[Почему уровни именно такие](STANDARD.ru.md#supervision).

Двум проверкам нужен диапазон коммитов, и у них может быть свой шаг:
...
- Пиньте версию, которую внедрили: `harnessimo@0.10.1`, а не диапазон. Отслеживание последней
