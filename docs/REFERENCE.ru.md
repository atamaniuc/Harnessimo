...
Один репозиторий запрещает service-role-ключ где-либо под `app/`: он обходит row-level
security, поэтому использование на клиенте — это утечка данных в одежде обычного импорта. Он
же запрещает захардкоженные цвета в компонентах, потому что дизайн-токены живут в одном
файле. Другой запрещает маркеры-заглушки в опубликованном контенте. Три правила, одна форма:
*этот шаблон, не в этих файлах, по этой причине.*
...
Причина обязательна, и в ней весь смысл. Граница, печатающая «совпало /service_role/»,
говорит, что случилось; следующему человеку нужно знать, что делать вместо этого.

**Включить:** `boundaries` со списком правил из `pattern`, `paths` и `reason` (плюс `allow`
для того единственного законного места). Этот репозиторий применяет одно к себе: импорт в
`src/`, который не `node:`-встроенный и не относительный, — это runtime-зависимость, которую
его же ограничения запрещают. Раньше это была проза, которую ревьюер должен был помнить,
теперь — строка, которая падает.
...
Один репозиторий держит `evals/thresholds.json` — recall, groundedness, валидность цитат, у
каждой свой пол — и роняет CI, когда оценка падает ниже. У другого гейт на обоснованность с
проходным баллом. Оба держат файл с полами вне досягаемости агента, и это несущая половина:
цикл, который может опустить себе проходной балл, оценивает сам себя.
...
Она не считает ваши метрики. Ваша команда пишет файл результатов, а проверка читает и
сравнивает. Метрика, у которой есть пол, но которую никто не измерил, тоже падает: шаг
подсчёта, который не запускался, — это не шаг подсчёта, который прошёл.

**Включить:** `thresholds`, назвав `results`, `floors` и `command`, которая создаёт
результаты. Если репозиторий уже объявил `locked.paths`, а файла с полами среди них нет,
проверка об этом скажет. Только полы: метрика, где меньше значит лучше, — то же правило
зеркально, а придумывать направление на нуле реальных случаев было бы гаданием.
