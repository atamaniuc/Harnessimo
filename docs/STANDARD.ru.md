...
## Сколько перепроверять { #supervision }

Все проверки выше отвечают на один вопрос: закончено ли. Ни одна не спрашивает, как этот
ответ был получен, — а один и тот же зелёный отчёт значит разное в зависимости от того, кто
смотрел. У человека, прочитавшего каждый шаг и запустившего `check`, есть два независимых
суждения, которые сошлись. У ночного прогона, закончившегося зелёным, — одно, и оно его
собственное.

Поэтому репозиторий объявляет, насколько плотно за работой смотрят, и уровень решает,
сколько перепроверяется.

| Уровень | Что означает | Что добавляет |
| --- | --- | --- |
| `watched` | человек читает каждый шаг по ходу дела | быстрые проверки — proof, tracks, tasks, queue, instructions, release |
| `reviewed` | за шагами никто не смотрел; диф прочитают | переперепроверку и проверку закрытых поверхностей |
| `unattended` | не смотрел никто | чистый выход и холодный старт |

Лестница и есть аргумент. Человек, читающий каждый шаг, всё равно пропустит утверждение в
доке, которое тихо перестало быть правдой. Человек, читающий только диф, не перезапустит
каждое «passing» и не заметит, как агент правит файл, который его оценивает. «Не смотрел
никто» означает, что мусор не заметит никто — и никто не заметит, что репозиторий перестал
запускаться из чистого клона.

**Уровень объявляется, а не угадывается.** Инструмент мог бы посмотреть на `CI`, на то,
терминал ли stdin, на форму истории коммитов, — и каждое из этого догадка о человеческом
внимании в одежде факта. Вместо этого проверяется, что объявление обеспечено: заявить
`unattended` в репозитории без проверки холодного старта — это прогон без надзора, за
заявлением которого ничего не стоит, и `check` скажет об этом, а не напечатает зелёное,
которое значит меньше, чем выглядит.
<!-- proof: src/autonomy.ts:unbacked -->

Это нижняя граница, а не настройка. `--autonomy` её поднимает и не может опустить: планка,
под которую прогон без надзора может себя уговорить, — не планка.
...
- **Наблюдаемости и инженерии циклов/графов** (лекции 11, 13, 14). Это слой выше. Несколько
  агентов на одном репозитории теперь учтены — полоса объявляет, что держит и чего ждёт, а ход
  не заканчивается при красной проверке, — но ничего здесь не планирует, не маршрутизирует и не
  надзирает за работающим циклом. Назвать пробел честнее, чем делать вид, что он закрыт.
- **Гигиены навыков и промптов.** Навык, чьё описание никогда не срабатывает, — настоящая
  проблема, но не та, которую эта библиотека видела дважды. Каждое правило здесь выросло из
  наблюдения, как два независимых репозитория пишут одно и то же руками; правило, построенное
  на документации одного вендора и нуле наблюдений, — догадка, а догадки это то, ради чего
  проверки и существуют.
