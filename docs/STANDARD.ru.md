...
| Лекция | Идея | Реализовано как |
|---|---|---|
| 02 · что такое харнес | пять подсистем, метафора кухни | раскладка `.harness/` |
| 03 · репозиторий как источник правды | того, чего нет в репозитории, не существует | `harnessimo proof`, `harnessimo cold-start` |
| 04 · один большой файл инструкций не работает | инструкция — это роутер | `harnessimo instructions` |
| 05 · преемственность между сессиями | PROGRESS, DECISIONS, протокол сессии | `.harness/4-state/`, `harnessimo tracks` |
| 06 · инициализация как отдельная фаза | развёрнутый старт | `harnessimo init` |
| 05 · преемственность между сессиями | состояние передают, а не ищут | `harnessimo brief`, SessionStart-хук |
| 07 · переусердствовать и недоделать | WIP = 1, объём работ принадлежит человеку | `harnessimo queue activate` |
| 08 · списки фич как примитивы | поведение + верификация + состояние | `harnessimo queue` |
| 09 · слишком раннее объявление победы | гейт на состояние passing, доказательства | `harnessimo queue verify`, `--reverify` |
| 10 · сквозной прогон как основание истины | доказательство — это реальный запуск | `harnessimo cold-start` |
| 12 · чистое состояние в конце сессии | без мусора, прогресс записан | `harnessimo clean-exit` |

Две вещи здесь **не** из курса: proof-маркеры и handoff-driven development. Обе пришли из
продакшн-репозиториев и описаны ниже в собственных разделах.
...
падает ли на нём команда или его ловят на ревью. В этом репозитории четыре из девяти —
review-only, и они об этом сообщают.
