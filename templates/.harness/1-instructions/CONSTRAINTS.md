...
1. **Never mark an item done without running its verification.** _(enforced: only
   `harness queue verify` writes evidence, and `harness queue check --reverify` re-runs
   every passing claim in CI)_
   Evidence, not confidence.

2. **Never hold more than one item active.** _(enforced: `harness queue activate`)_
...
3. **Never report work complete without linking the evidence.** _(enforced: an item in a
   terminal state with no evidence fails `harness queue check`)_
