 ...
 */
export function trackLines(text) {
  const lines = [];
  let inFence = false;
 ...
 */
export function checkTracks(text, resolver, filePath = "specs/TRACKS.md") {
  const problems = [];
 ...
 * @param {import("./proof.mjs").Resolver} resolver
 * @param {string} filePath
 * @param {string} log  where a closed track's outcome lives, named in the failure message
 * @returns {import("./proof.mjs").Problem[]}
 */
export function checkHandoffRefs(text, resolver, filePath, log = "specs/TRACKS-LOG.md") {
  const problems = [];
  let inFence = false;
  text.split("\n").forEach((lineText, index) => {
    if (lineText.trimStart().startsWith("```")) {
 ...
 * @returns {string[]}
 */
export function liveTrackSpecDirs(tracksText, specsDir = "specs") {
