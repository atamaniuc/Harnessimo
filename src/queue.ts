...
import type { FeatureList, QueueItem, RunResult } from "./types.ts";

import { readiness, doneness, formatViolations } from "./readiness.ts";
...
const DEFAULT_TERMINAL: Record<string, string> = { code: "passing" };

function terminalState(item: QueueItem, terminalByKind: Record<string, string>): string {
...
export function formatStatus(list: FeatureList): string {
  ...
  for (const i of list.items) byState.set(i.state, [...(byState.get(i.state) ?? []), i]);
  const lines = [];
  for (const state of [...order, ...[...byState.keys()].filter((s) => !order.includes(s))]) {
    const items = byState.get(state) ?? [];
    if (items.length === 0) continue;
    lines.push(`\n${state} (${items.length})`);
    for (const i of items) lines.push(`  ${i.id.padEnd(28)} ${i.phase ? `phase ${i.phase}` : ""}`.trimEnd());
  }
...
export function activate(list: FeatureList, id: string): QueueItem {
  ...
  if (active.length >= wipLimit) {
    throw new HarnessError(
      `WIP limit reached — "${active[0]?.id}" is already active`,
      "finish or block that item first; split attention is what produces work started everywhere and finished nowhere",
    );
  }
  ...
  if (queued >= queueLimit) {
    throw new HarnessError(
      `verification queue is full (${queued}/${queueLimit})`,
      "let the gates drain before starting more; a backlog of ungated work is not progress",
    );
  }
 ...
 *
 */
export function verifyItem(list: FeatureList, id: string, runner: (command: string) => { ok: boolean, output: string }, options: { terminalByKind?: Record<string,string> } = {}) {
 ...
 */
export function checkQueue(list: FeatureList, options: { reverify?: boolean, runner?: (command: string) => { ok: boolean, output: string } } = {}): string[] {
  ...
  if (active.length > wipLimit) {
    problems.push(
      `WIP limit exceeded: ${active.length} active (${active.map((i) => i.id).join(", ")})\n` +
  ...
  if (queued.length > queueLimit) {
    problems.push(
      `verification queue over limit: ${queued.length}/${queueLimit}\n` +
  ...
  if (reverify && runner) {
    for (const item of list.items.filter((i) => i.state === "passing" || i.state === "done")) {
      ...
      if (!ok) {
        problems.push(
          `"${item.id}" claims ${item.state} but its verification fails now\n` +
...
/** @param text @param n */
export function lastLines(text: string, n: number = 6) {
  return String(text)
    .trim()
    .split("\n")
