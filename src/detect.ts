...
/**
 */
export function detectConfig(repo: Repo): {
  ...
  // Written section by section as the repository is read, so the shape is not
  // known until the end. It is validated on the way back in by loadConfig.
  const config: DetectedConfig = {
    $schema: "https://raw.githubusercontent.com/atamaniuc/Harnessimo/main/schema/harnessimo.config.schema.json",
  };
  ...
  if (migrations) {
    (config.docs as { migrations?: string }).migrations = migrations;
  ...
  const commandsToRun = [setup, checkCommand].filter((c): c is string => Boolean(c));
...
function pickCheckCommand(repo: Repo, commands: Record<string, string>): string | null {
  for (const [prefix, file] of Object.entries(commands)) {
    ...
    const names = file.endsWith("package.json")
      ? Object.keys((safeJson(text).scripts as Record<string, unknown>) ?? {})
      : /\.ya?ml$/.test(file)
