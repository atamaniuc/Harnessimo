...
const isWordMarker = (marker: string): boolean => /^[A-Za-z0-9_]+$/.test(marker);
 ...
 */
function isAnnotation(lineText: string, marker: string): boolean {
 ...
 */
export function progressProblems({
  changed,
  progressFile,
  codePrefixes = [],
  changedLines = null,
 ...
 */
export function instructionProblems(files: { path: string, text: string }[], limits: Record<string, number>): Problem[] {
  const problems: Problem[] = [];
  for (const file of files) {
