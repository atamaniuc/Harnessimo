...
/**
 * @typedef {{ what: string, why: string, fix: string }} Violation
 * @typedef {{ id: string, kind?: string, behavior: string, verification: string,
 *             state: string, evidence: string | null, depends_on?: string[] }} Item
 */
 ...
 * @returns {Violation[]}
 */
export function readiness(item, all) {
  const problems = [];
 ...
 *
 * @param {Item} item
 * @param {boolean} verificationPassed
 * @returns {Violation[]}
 */
export function doneness(item, verificationPassed) {
  const problems = [];
 ...
 */
export function formatViolations(id, violations) {
