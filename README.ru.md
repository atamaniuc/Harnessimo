...
Две половины, которые нужны друг другу. **SDD**: работа — это лан, нумерованная спека,
критерии приёмки которой умеют запускаться. **Харнесс**: одиннадцать проверок, которые
отказываются считать этот лан законченным, пока они не проходят. Спеки без принуждения —
картотека; принуждение без спек — не с чем сверять.

`harnessimo track new <slug>` открывает лан, `harnessimo check` решает, закончен ли он.
Работает и на пустом репозитории, и на том, где пятнадцать лет истории: чтобы начать, ничего
переписывать не нужно, а одна включённая проверка — уже реальное улучшение.
...
<p align="center">
  <img src="https://raw.githubusercontent.com/atamaniuc/Harnessimo/main/docs/assets/demo.png"
       alt="Терминал: harnessimo init пишет конфиг, check проходит, кто-то переименовывает функцию, на которую ссылался README, и check падает с файлом, строкой и утверждением"
       width="860">
</p>

Это никто не рисовал: `npm run demo` запускает эти команды во временном репозитории и
записывает то, что они напечатали. <!-- proof: scripts/record-demo.ts -->
<!-- demo:end -->

<!-- install:start -->
```bash
pnpm add -D harnessimo
pnpm exec harnessimo init      # читает ваш репозиторий и пишет конфиг, который уже проходит
pnpm exec harnessimo check     # эту команду ставите в CI
```
...
Все четыре команды были запущены на этом релизе до того, как попали сюда.
...
Полное сравнение, слой за слоем — [Зачем это](https://atamaniuc.github.io/Harnessimo/ru/WHY/).
...
Подробно: [Внедрение в существующий репозиторий](https://atamaniuc.github.io/Harnessimo/ru/GUIDE/#adopting).
...
- **Он живёт по собственному стандарту.** Десять из одиннадцати применяются к этому
...
Этот репозиторий применяет к себе десять из своих одиннадцати проверок, в том числе из
свежего клона. Одиннадцатой, `thresholds`, нужна измеряемая метрика, а инструменту, которому
нечего измерять, нечего и держать выше пола — `harnessimo doctor` говорит это прямо, а не
намёком.
<!-- proof: npm run check -->
...
[MIT](LICENSE). Форкайте, вендорите, используйте коммерчески. Если улучшите правило — пришлите
